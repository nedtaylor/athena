...
!-------------------------------------------------------------------------------
  ...
  call input(1,1)%allocate([width, width, width, num_channels, 1], source=0._real32)
  ...
  if( any( &
       ...
       [output_width,output_width,output_width,num_filters2,1] &
  ) )then
     success = .false.
...
! check gradients
!-------------------------------------------------------------------------------
  allocate(data_tmp(width, width, width, num_channels, 1))
  data_tmp = 0.E0
  ...
  type is(conv3d_layer_type)
     ...
     if(size(gradients).ne.&
          (kernel_size2**3*num_filters1 + 1) * num_filters2)then
        success = .false.
        write(0,*) "conv3d network gradients size should be ", &
             ( kernel_size2**3 * num_filters1 + 1 ) * num_filters2
     end if
