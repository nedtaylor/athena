program test_spectral_filter_layer
  !! Tests for the Spectral Filter layer
  use athena, only: &
...
!-------------------------------------------------------------------------------
  ...
  layer1 = spectral_filter_layer_type(num_inputs=4, num_outputs=8, num_modes=3)

  if(layer1%name .ne. 'spectral_filter')then
     success = .false.
     write(0,*) 'spectral_filter layer has wrong name: '//trim(layer1%name)
  end if

  if(any(layer1%input_shape .ne. [4]))then
     success = .false.
     write(0,*) 'spectral_filter layer has wrong input_shape'
  end if

  if(any(layer1%output_shape .ne. [8]))then
     success = .false.
     write(0,*) 'spectral_filter layer has wrong output_shape'
  end if

  select type(layer1)
  type is(spectral_filter_layer_type)
     if(layer1%num_inputs .ne. 4)then
        success = .false.
        write(0,*) 'spectral_filter layer has wrong num_inputs'
     end if
     if(layer1%num_outputs .ne. 8)then
        success = .false.
        write(0,*) 'spectral_filter layer has wrong num_outputs'
     end if
     if(layer1%num_modes .ne. 3)then
        success = .false.
        write(0,*) 'spectral_filter layer has wrong num_modes'
     end if
     if(layer1%activation%name .ne. 'none')then
        success = .false.
        write(0,*) 'spectral_filter layer has wrong default activation: '// &
             layer1%activation%name
     end if
     ! num_params = M + n_out * n_in + n_out (bias)
     !            = 3 + 32 + 8 = 43
     if(layer1%num_params .ne. 43)then
        success = .false.
        write(0,'("spectral_filter layer has wrong num_params: ",I0)') &
             layer1%num_params
     end if
     ! Check basis matrices are allocated
     if(.not.layer1%forward_basis%allocated)then
        success = .false.
        write(0,*) 'spectral_filter forward_basis not allocated'
     end if
     if(.not.layer1%inverse_basis%allocated)then
        success = .false.
        write(0,*) 'spectral_filter inverse_basis not allocated'
     end if
  class default
     success = .false.
...
!-------------------------------------------------------------------------------
  ...
  layer2 = spectral_filter_layer_type(num_outputs=6, num_modes=2)
  call layer2%init(layer1%output_shape)

  if(any(layer2%input_shape .ne. [8]))then
     success = .false.
     write(0,*) 'deferred spectral_filter layer has wrong input_shape'
  end if
  if(any(layer2%output_shape .ne. [6]))then
     success = .false.
     write(0,*) 'deferred spectral_filter layer has wrong output_shape'
  end if
...
!-------------------------------------------------------------------------------
  ...
  layer3 = spectral_filter_layer_type( &
  ...
  type is(spectral_filter_layer_type)
     if(layer3%use_bias)then
        success = .false.
        write(0,*) 'no-bias spectral_filter layer has use_bias = T'
     end if
     ! num_params = M + n_out * n_in = 2 + 16 = 18 (no bias)
     if(layer3%num_params .ne. 18)then
        success = .false.
...
!-------------------------------------------------------------------------------
  ...
  layer1 = spectral_filter_layer_type( &
  ...
  type is(spectral_filter_layer_type)
     if(layer1%activation%name .ne. 'relu')then
        success = .false.
        write(0,*) 'spectral_filter layer has wrong activation: '// &
             layer1%activation%name
     end if
  class default
     success = .false.
...
!-------------------------------------------------------------------------------
  ...
  layer2 = spectral_filter_layer_type( &
...
!-------------------------------------------------------------------------------
  ...
  open(newunit=unit, file='test_spectral_filter_layer.tmp', &
       status='replace', action='write')
  write(unit,'("SPECTRAL_FILTER")')
  call layer1%print_to_unit(unit)
  write(unit,'("END SPECTRAL_FILTER")')
  close(unit)

  open(newunit=unit, file='test_spectral_filter_layer.tmp', &
       status='old', action='read')
  read(unit,*)   ! skip SPECTRAL_FILTER header line
  read_layer = read_spectral_filter_layer(unit)
  close(unit)

  select type(read_layer)
  type is(spectral_filter_layer_type)
     if(read_layer%name .ne. 'spectral_filter')then
        success = .false.
     ...
     type is(spectral_filter_layer_type)
        if(any(abs(read_layer%params(1)%val(:,1) - &
           ...
           success = .false.
           write(0,*) 'read layer w_s weights differ from original'
        end if
  ...
  class default
     success = .false.
     write(0,*) 'read layer is not spectral_filter_layer_type'
  end select

  open(newunit=unit, file='test_spectral_filter_layer.tmp', status='old')
...
!-------------------------------------------------------------------------------
  write(*,*) "----------------------------------------"
  if(success)then
     write(*,*) 'test_spectral_filter_layer passed all tests'
  else
     write(0,*) 'test_spectral_filter_layer failed one or more tests'
     stop 1
  end if
...
!-------------------------------------------------------------------------------
  subroutine compare_layers(layer1, layer2, success, layer3)
    type(spectral_filter_layer_type), intent(in) :: layer1, layer2
    logical, intent(inout) :: success
    type(spectral_filter_layer_type), optional, intent(in) :: layer3

    if(layer1%num_inputs .ne. layer2%num_inputs)then
       success = .false.
       write(0,*) 'spectral_filter layer comparison: wrong num_inputs'
    end if
    if(layer1%activation%name .ne. 'sigmoid')then
       success = .false.
       write(0,*) 'spectral_filter layer comparison: wrong activation: '// &
            layer1%activation%name
    end if
    if(present(layer3))then
       if( &
          ...
          if(any(abs( &
             ...
             success = .false.
             write(0,*) 'spectral_filter layer: w_s gradient sum mismatch'
          end if
       end if
       if( &
          ...
          if(any(abs( &
             ...
             success = .false.
             write(0,*) 'spectral_filter layer: W gradient sum mismatch'
          end if
       end if
    end if
  end subroutine compare_layers

end program test_spectral_filter_layer
