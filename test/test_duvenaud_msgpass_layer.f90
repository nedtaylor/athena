program test_duvenaud_msgpass_layer
  use athena, only: &
  ...
  integer, parameter :: num_vertex_features = 10, num_time_steps = 3, num_outputs = 5
...
!!!-----------------------------------------------------------------------------
  msgpass_layer = duvenaud_msgpass_layer_type( &
  ...
  !! check layer name
  if(.not. msgpass_layer%name .eq. 'duvenaud')then
     success = .false.
     write(0,*) 'duvenaud layer has wrong name'
  end if
...
!!!-----------------------------------------------------------------------------
  ...
  type is(duvenaud_msgpass_layer_type)
     ...
     if(msgpass_layer%num_vertex_features(1) .ne. num_vertex_features)then
        success = .false.
        write(0,*) 'duvenaud layer has wrong input features'
     end if

     !! check number of outputs
     if(msgpass_layer%num_outputs .ne. num_outputs)then
        success = .false.
        write(0,*) 'duvenaud layer has wrong output features'
     end if

     !! check number of time steps
     if(msgpass_layer%num_time_steps .ne. num_time_steps)then
        success = .false.
        write(0,*) 'duvenaud layer has wrong number of time steps'
     end if
  end select
...
!!!-----------------------------------------------------------------------------
  ...
  call graph(1)%set_num_vertices(num_vertices, num_vertex_features)
  call graph(1)%set_num_edges(num_edges)

  ! Set up some dummy vertex features
  graph(1)%is_sparse = .true.
  allocate(graph(1)%vertex_features(num_vertex_features, num_vertices))
  ...
  block
    ...
    allocate(graph(1)%edge_features(num_edge_features, num_edges), source = 0.0)
  end block
...
!!!-----------------------------------------------------------------------------
  ...
  class is(learnable_layer_type)
     ...
     if(size(params) .eq. 0)then
        success = .false.
        write(0,*) 'duvenaud layer has wrong number of parameters'
     end if
     params = 1.E0
     ...
     if(any(abs(params - 1.E0).gt.tol))then
        success = .false.
        write(0,*) 'duvenaud layer has wrong parameters'
     end if
     ...
     if(size(params) .eq. 0)then
        success = .false.
        write(0,*) 'duvenaud layer has wrong number of gradients'
     end if
     params = 1.E0
...
!!!-----------------------------------------------------------------------------
  ...
  open(newunit=unit, file='test_duvenaud_msgpass_layer.tmp', &
       status='replace', action='write')

  ! Write layer to file
  write(unit,'("DUVENAUD_MSGPASS")')
  call msgpass_layer%print_to_unit(unit)
  write(unit,'("END DUVENAUD_MSGPASS")')
  close(unit)
  ...
  read_layer = read_duvenaud_msgpass_layer(unit)
  close(unit)
  ...
  class default
     success = .false.
     write(0,*) 'read layer is not duvenaud_msgpass_layer_type'
  end select

  ! Clean up temporary file
  open(newunit=unit, file='test_duvenaud_msgpass_layer.tmp', status='old')
...
!!!-----------------------------------------------------------------------------
  write(*,*) "----------------------------------------"
  if(success)then
     write(*,*) 'test_duvenaud_msgpass_layer passed all tests'
  else
     write(0,*) 'test_duvenaud_msgpass_layer failed one or more tests'
     stop 1
  end if

end program test_duvenaud_msgpass_layer
