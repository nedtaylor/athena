program test_laplace_nop_layer
...
!-------------------------------------------------------------------------------
  ...
  layer1 = laplace_nop_layer_type(num_inputs=4, num_outputs=8, num_modes=3)
  ...
  select type(layer1)
  type is(laplace_nop_layer_type)
...
!-------------------------------------------------------------------------------
  ...
  layer2 = laplace_nop_layer_type(num_outputs=6, num_modes=2)
...
!-------------------------------------------------------------------------------
  ...
  open(newunit=unit, file='test_laplace_nop_layer.tmp', &
  ...
  read_layer = read_laplace_nop_layer(unit)
  close(unit)
  ...
  class default
     success = .false.
     write(0,*) 'read layer is not laplace_nop_layer_type'
  end select

  open(newunit=unit, file='test_laplace_nop_layer.tmp', status='old')
...
!-------------------------------------------------------------------------------
  write(*,*) "----------------------------------------"
  if(success)then
     write(*,*) 'test_laplace_nop_layer passed all tests'
  else
     write(0,*) 'test_laplace_nop_layer failed one or more tests'
     stop 1
  end if
...
!-------------------------------------------------------------------------------
  subroutine compare_layers(layer1, layer2, success, layer3)
    type(laplace_nop_layer_type), intent(in) :: layer1, layer2
    logical, intent(inout) :: success
    type(laplace_nop_layer_type), optional, intent(in) :: layer3
...
end program test_laplace_nop_layer
