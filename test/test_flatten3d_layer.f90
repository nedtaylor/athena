program test_flatten3d_layer
  use constants, only: real12
  use flatten3d_layer, only: flatten3d_layer_type
  implicit none

  type(flatten3d_layer_type) :: flatten_layer
  ...
  !! set up flatten3d layer
  flatten_layer = flatten3d_layer_type( &
       input_shape = [width, width, width, num_channels], &
       batch_size = batch_size)

  !! check layer type
  if(.not. flatten_layer%name .eq. 'flatten3d')then
     success = .false.
     write(0,*) 'flatten3d layer has wrong name'
  end if

  !! check input shape
  if(any(flatten_layer%input_shape .ne. &
       [width, width, width, num_channels]))then
     success = .false.
     write(0,*) 'flatten3d layer has wrong input_shape'
  end if

  !! check output shape
  if(any(flatten_layer%output_shape .ne. [width*width*width*num_channels]))then
     success = .false.
     write(0,*) 'flatten3d layer has wrong output_shape'
  end if

  !! check batch size
  if(flatten_layer%batch_size .ne. 1)then
     success = .false.
     write(0,*) 'flatten3d layer has wrong batch size'
  end if

  !! initialise sample input
  allocate(input_data(width, width, width, num_channels, batch_size), &
       source = 0.0)
  ...
  if(any(abs(pack(input_data(:,:,:,:,1),mask=.true.) - output(:,1)).gt.tol))then
     success = .false.
     write(0,*) 'flatten3d layer forward pass incorrect'
   end if
  ...
  if(any(abs(flatten_layer%di - input_data).gt.tol))then
    success = .false.
    write(0,*) 'flatten3d layer backward pass incorrect'
  end if
...
!!!-----------------------------------------------------------------------------
  write(*,*) "----------------------------------------"
  if(success)then
     write(*,*) 'test_flatten3d_layer passed all tests'
  else
     write(0,*) 'test_flatten3d_layer failed one or more tests'
     stop 1
  end if

end program test_flatten3d_layer