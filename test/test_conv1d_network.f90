...
!!!-----------------------------------------------------------------------------
  ...
  if(network%num_layers.ne.3)then
    success = .false.
    write(*,*) "conv1d network should have 3 layers"
  end if
  ...
  if(any(shape(output).ne.[width-4,num_filters2,1]))then
     success = .false.
     write(*,*) "conv1d network output shape should be [28,28,32]"
  end if
...
!!!-----------------------------------------------------------------------------
  ...
  call network%forward(input_data)
  output_reshaped = reshape(output, [(width-4)*num_filters2,1])
  ...
  type is(conv1d_layer_type)
     ...
     gradients_weight = &
          reshape(&
          gradients(:kernel_size*num_filters1*num_filters2), &
          [kernel_size,num_filters1,num_filters2])
     gradients_bias = &
          gradients(kernel_size*num_filters1*num_filters2+1:)
     if(size(gradients).ne.&
          (kernel_size*num_filters1 + 1) * num_filters2)then
        success = .false.
        write(*,*) "conv1d network gradients size should be ", &
             ( kernel_size * num_filters1 + 1 ) * num_filters2
     end if
     if(any(abs(gradients).lt.1.E-6))then
        success = .false.
        write(*,*) "conv1d network gradients should not be zero"
     end if
     if(any(abs(gradients_weight(1,:,:) - &
          gradients_weight(1,1,1)).gt.1.E-6))then
        success = .false.
        write(*,*) "conv1d network gradients first column should be equivalent"
     end if
     if(any(abs(gradients_weight(2,:,:) - &
          gradients_weight(2,1,1)).gt.1.E-6))then
        success = .false.
        write(*,*) "conv1d network gradients second column should be equivalent"
     end if
     if(any(abs(gradients_weight(3,:,:) - &
          gradients_weight(3,1,1)).gt.1.E-6))then
        success = .false.
        write(*,*) "conv1d network gradients third column should be equivalent"
     end if
     if(any(abs(gradients_bias(:)-gradients_bias(1)).gt.1.E-6))then
        success = .false.
        write(*,*) "conv1d network gradients bias should be equivalent"
     end if
  class default
     success = .false.
     write(*,*) "conv1d network layer should be conv1d_layer_type"
  end select
...
!!!-----------------------------------------------------------------------------
  write(*,*) "----------------------------------------"
  if(success)then
     write(*,*) 'test_conv1d_network passed all tests'
  else
     write(0,*) 'test_conv1d_network failed one or more tests'
     stop 1
  end if
