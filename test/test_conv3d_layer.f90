program test_conv3d_layer
  ...
  if(.not. conv_layer%name .eq. 'conv3d')then
     success = .false.
     write(0,*) 'conv3d layer has wrong name'
  end if
  ...
  type is(conv3d_layer_type)
     ...
     if(allocated(conv_layer%input_shape))then
        success = .false.
        write(0,*) 'conv3d layer shape should not be allocated yet'
     end if
  class default
     success = .false.
...
!!!-----------------------------------------------------------------------------
  ...
  class is(learnable_layer_type)
     ...
     if(size(params) .eq. 0)then
        success = .false.
        write(0,*) 'conv3d layer has wrong number of parameters'
     end if
     params = 1.E0
     ...
     if(any(abs(params - 1.E0).gt.tol))then
        success = .false.
        write(0,*) 'conv3d layer has wrong parameters'
     end if
     ...
     if(any(abs(params - 1.E0).gt.tol))then
        success = .false.
        write(0,*) 'conv3d layer has wrong gradients'
     end if
...
!!!-----------------------------------------------------------------------------
  ...
  if(success)then
     ...
     write(0,*) 'test_conv3d_layer failed one or more tests'
     stop 1
  end if
