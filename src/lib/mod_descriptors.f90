...
!!!#############################################################################
module descriptor_layer
  use constants, only: real12
  use base_layer, only: base_layer_type
  use custom_types, only: initialiser_type
  implicit none
  
  ! do we store the set of points in the layer or feed it in every time?
  ! probably read into the layer and store in there
  ! the output will be a list of radius values

  ! The filters could be:
  ! - rotational circular filters applied to the discrete set of points
  !   - the output would be circular images
  !   - the images either summed for each point after the layer or at the end of the system
  !   - unique ones for each species later on?
  ! - a set of bessel function
  !   - the inputs also include locations of neighbouring points
  !   - the output would be the images again
  !   - or would they be the values of the bessel functions?
  ! GOT IT:
  !   - input is the charge density
  !   - sample points that fall within the cutoff radius (up to a certain percentage of density)
  !   - apply spherical harmonics at each atom, learning on the points.
  !     - opt: apply spherical harmonics for neighbour atoms also
  !     - opt: could be a sum of gaussians and plane waves instead
  !     - opt: set up triplets of atoms and learn from those (probably only within the vertices made by those three, but that's just a 2D space then)
  !       - this defines an equivariant theta and phi for each set (i.e. theta = angle from atom 2 to atom 3, phi = angle away from plane normal vector)
  !   - output is the set of constants applied to the summation of spherical harmonics


  type, extends(base_layer_type)) :: descriptor_layer_type
     !! alpha = atom, beta = species, n = radial, l = angular, m = angular
     !! output(alpha, beta, n,l) = sum(m, coeff(alpha, beta, n, l, m)**2.0)
     real(real12), allocatable, dimension(:,:) :: lattice !! lattice vectors
     real(real12), allocatable, dimension(:,:) :: position !! position of the atoms, make it species independent for now
     real(real12), allocatable, dimension(:,:,:,:,:) :: coeff !! expansion coefficient (alpha, beta, n, l, m)
   ...
   contains
     procedure, pass(this) :: get_params => get_params_descriptor
     procedure, pass(this) :: set_params => set_params_descriptor
     procedure, pass(this) :: get_gradients => get_gradients_descriptor
     procedure, pass(this) :: set_gradients => set_gradients_descriptor
     procedure, pass(this) :: get_output => get_output_descriptor

     procedure, pass(this) :: init => init_descriptor
   !   procedure, pass(this) :: set_batch_size => set_batch_size_descriptor
   !   procedure, pass(this) :: print => print_descriptor
  ...
  end type descriptor_layer_type

  
!!!-----------------------------------------------------------------------------
!!! interface for layer set up
!!!-----------------------------------------------------------------------------
  interface descriptor_layer_type
     module function layer_setup( &
       ...
       type(descriptor_layer_type) :: layer
     end function layer_setup
  end interface descriptor_layer_type


  private
  public :: descriptor_layer_type
  public :: read_descriptor_layer
...
!!!#############################################################################
  pure subroutine forward_rank(this, input)
    implicit none
    class(descriptor_layer_type), intent(inout) :: this
...
!!!#############################################################################
  module function layer_setup( &
    ...
    if(present(calc_input_gradients))then
       ...
       write(*,*) "DESCRIPTOR input gradients turned off"
    else
    ...
    write(*,'("DESCRIPTOR activation function: ",A)') trim(t_activation_function)
...
!!! initialise layer
!!!#############################################################################
  subroutine init_descriptor(this, input_shape, batch_size, verbose)
...
!!! set batch size
!!!#############################################################################
  subroutine set_batch_size_descriptor(this, batch_size, verbose)
   implicit none
   class(descriptor_layer_type), intent(inout) :: this
 ...
 end subroutine set_batch_size_descriptor
!!!#############################################################################
...
!!! print layer to file
!!!#############################################################################
  subroutine print_descriptor(this, file)
    implicit none
    class(descriptor_layer_type), intent(in) :: this
...
!!!#############################################################################
  function read_descriptor_layer(unit, verbose) result(layer)
    ...
    class(descriptor_layer_type), allocatable :: layer
    ...
    tag_loop: do
       ...
       if(stat.ne.0)then
         write(0,*) "ERROR: file encountered error (EoF?) before END DESCRIPTOR"
          stop "Exiting..."
       end if
       ...
       !!-----------------------------------------------------------------------
       if(trim(adjustl(buffer)).eq."END DESCRIPTOR")then
          backspace(unit)
          exit tag_loop
       end if
    ...
    if(.not.found_weights)then
      write(0,*) "WARNING: WEIGHTS card in DESCRIPTOR not found"
   else
       do l=1,num_filters
    ...
    !!--------------------------------------------------------------------------
    read(unit,'(A)') buffer
    if(trim(adjustl(buffer)).ne."END DESCRIPTOR")then
       write(*,*) trim(adjustl(buffer))
       stop "ERROR: END DESCRIPTOR not where expected"
    end if

  end function read_descriptor_layer
!!!#############################################################################
