...
!!!#############################################################################
module flatten1d_layer
  use constants, only: real12
  use base_layer, only: flatten_layer_type
  implicit none
  
  
  type, extends(flatten_layer_type) :: flatten1d_layer_type
     real(real12), allocatable, dimension(:,:,:) :: di
   contains
     procedure, pass(this) :: init => init_flatten1d
     procedure, pass(this) :: set_batch_size => set_batch_size_flatten1d
     procedure, pass(this) :: forward  => forward_rank
     procedure, pass(this) :: backward => backward_rank
  end type flatten1d_layer_type

  interface flatten1d_layer_type
     module function layer_setup(input_shape, batch_size, num_addit_outputs) &
       ...
       type(flatten1d_layer_type) :: layer
     end function layer_setup
  end interface flatten1d_layer_type

  
  private
  public :: flatten1d_layer_type
...
!!!#############################################################################
  pure subroutine forward_rank(this, input)
    implicit none
    class(flatten1d_layer_type), intent(inout) :: this
...
!!! initialise layer
!!!#############################################################################
  subroutine init_flatten1d(this, input_shape, batch_size, verbose)
    implicit none
    class(flatten1d_layer_type), intent(inout) :: this
...
!!! set batch size
!!!#############################################################################
  subroutine set_batch_size_flatten1d(this, batch_size, verbose)
    implicit none
    class(flatten1d_layer_type), intent(inout) :: this
  ...
  end subroutine set_batch_size_flatten1d
 !!!#############################################################################

end module flatten1d_layer
!!!#############################################################################
