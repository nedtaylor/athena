...
!###############################################################################
  subroutine fill_edge_region_1d(input, output)
    ...
    do f = 1, output%indices(3)
       do concurrent( s = 1:size(output%val, dim=2), &
            m = 1:output%shape(2) )
...
!-------------------------------------------------------------------------------
  subroutine accumulate_edge_gradients_1d(input, output)
    ...
    pad_size = input%indices(2)

    do f = 1, input%indices(3)
       do concurrent( s = 1:size(output%val, dim=2), &
            m = 1:output%shape(2) )
          select case(input%indices(1))
          case(3, 4) ! circular or reflection
             step = merge(1, -1, input%indices(1) .eq. 3)
             do i = 1, pad_size
                idx_in = input%adj_ja(1,(f-1)*2 + 1) + step * (i - 1) + &
                     (m-1)*input_size
                idx_out = input%adj_ja(2,(f-1)*2 + 1) + i - 1 + &
                     (m-1)*output_size
                output%val(idx_in, s) = output%val(idx_in, s) + &
                     input%val(idx_out, s)
             end do
          case(5) ! replication
             idx_in = input%adj_ja(1,(f-1)*2 + 1) + (m-1)*input_size
             do i = 1, pad_size
                idx_out = input%adj_ja(2,(f-1)*2 + 1) + i - 1 + &
                     (m-1)*output_size
                output%val(idx_in, s) = output%val(idx_in, s) + &
...
!###############################################################################
  subroutine fill_corner_region_2d(input, output)
    ...
    do f = 1, output%indices(5)
       orig(1,1:2) = output%adj_ja(1,(f-1)*4 + 1 + idx_shift:(f-1)*4 + 2 + idx_shift)
       orig(2,1:2) = output%adj_ja(1,(f-1)*4 + 3 + idx_shift:(f-1)*4 + 4 + idx_shift)
       dest(1,1:2) = output%adj_ja(2,(f-1)*4 + 1 + idx_shift:(f-1)*4 + 2 + idx_shift)
       dest(2,1:2) = output%adj_ja(2,(f-1)*4 + 3 + idx_shift:(f-1)*4 + 4 + idx_shift)
...
!-------------------------------------------------------------------------------
  subroutine fill_edge_region_2d(input, output)
    ...
    do f = 1, output%indices(4)
       ...
       orig(1,1:2) = output%adj_ja(1,(f-1)*4 + 1:(f-1)*4 + 2)
       orig(2,1:2) = output%adj_ja(1,(f-1)*4 + 3:(f-1)*4 + 4)
       dest(1,1:2) = output%adj_ja(2,(f-1)*4 + 1:(f-1)*4 + 2)
       dest(2,1:2) = output%adj_ja(2,(f-1)*4 + 3:(f-1)*4 + 4)
...
!-------------------------------------------------------------------------------
  subroutine accumulate_corner_gradients_2d(input, output)
...
!-------------------------------------------------------------------------------
  subroutine accumulate_edge_gradients_2d(input, output)
    ...
    do f = 1, output%indices(4)
       ...
       do concurrent( s = 1:size(output%val, dim=2) )
          select case(input%indices(1))
          case(3, 4) ! circular or reflection
             step1 = merge(-1, 1, input%indices(1) .eq. 4 .and. idim .eq. 1)
             step2 = merge(-1, 1, input%indices(1) .eq. 4 .and. idim .eq. 2)
             do j = dest(1,2), dest(2,2)
                do i = dest(1,1), dest(2,1)
                   idx_out = i + (j-1) * output_h
                   idx_in = orig(1,1) + step1 * (i - dest(1,1)) + &
...
!###############################################################################
  subroutine fill_corner_region_3d(input, output)
    ...
    do f = 1, output%indices(7)
       orig(1,1:3) = output%adj_ja(1,(f-1)*6 + 1 + idx_shift:(f-1)*6 + 3 + idx_shift)
       orig(2,1:3) = output%adj_ja(1,(f-1)*6 + 4 + idx_shift:(f-1)*6 + 6 + idx_shift)
       dest(1,1:3) = output%adj_ja(2,(f-1)*6 + 1 + idx_shift:(f-1)*6 + 3 + idx_shift)
       dest(2,1:3) = output%adj_ja(2,(f-1)*6 + 4 + idx_shift:(f-1)*6 + 6 + idx_shift)
...
!-------------------------------------------------------------------------------
  subroutine fill_edge_region_3d(input, output)
    ...
    do f = 1, output%indices(5)
       ...
       orig(1,1:3) = output%adj_ja(1,(f-1)*6 + 1:(f-1)*6 + 3)
       orig(2,1:3) = output%adj_ja(1,(f-1)*6 + 4:(f-1)*6 + 6)
       dest(1,1:3) = output%adj_ja(2,(f-1)*6 + 1:(f-1)*6 + 3)
       dest(2,1:3) = output%adj_ja(2,(f-1)*6 + 4:(f-1)*6 + 6)
       ...
       do concurrent( s = 1:size(output%val, dim=2) )
          ...
          case(5) ! replication
             ...
             case(2) ! Edge along dimension 2
                do k = dest(1,3), dest(2,3)
                   idx_in = orig(1,1) + (k - dest(1,3)) * input_h * input_w
                   do j = dest(1,2), dest(2,2)
                      do i = dest(1,1), dest(2,1)
                         idx_out = i + (j-1) * output_h + &
                              (k-1) * output_h * output_w
                   ...
                   do j = dest(1,2), dest(2,2)
                      do i = dest(1,1), dest(2,1)
                         idx_out = i + (j-1) * output_h + &
                              (k-1) * output_h * output_w
                         output%val(idx_out, s) = &
                              input%val(idx_in + i - dest(1,1) + &
                                   (j - dest(1,2)) * input_h, s)
                      end do
                   end do
                end do
             end select
                   ...
                   do j = 1, input_w
                      do i = dest(1,1), dest(2,1)
                         idx_out = i + (j-1) * output_h + &
                              (k-1) * output_h * output_w
                         output%val(idx_out, s) = &
                              input%val(idx_in + (j-1) * input_h + &
                                   (k-1) * input_h * input_w, s)
                      end do
                   end do
                end do
...
!-------------------------------------------------------------------------------
  subroutine accumulate_corner_gradients_3d(input, output)
    ...
    idx_shift = ( input%indices(5) + input%indices(6) ) * 6
    do f = 1, output%indices(7)
       orig(1,1:3) = input%adj_ja(1,(f-1)*6 + 1 + idx_shift:(f-1)*6 + 3 + idx_shift)
       orig(2,1:3) = input%adj_ja(1,(f-1)*6 + 4 + idx_shift:(f-1)*6 + 6 + idx_shift)
       dest(1,1:3) = input%adj_ja(2,(f-1)*6 + 1 + idx_shift:(f-1)*6 + 3 + idx_shift)
       dest(2,1:3) = input%adj_ja(2,(f-1)*6 + 4 + idx_shift:(f-1)*6 + 6 + idx_shift)
...
!-------------------------------------------------------------------------------
  subroutine accumulate_edge_gradients_3d(input, output)
    ...
    do f = 1, output%indices(5)
       idim = output%indices(7 + f)
       orig(1,1:3) = input%adj_ja(1,(f-1)*6 + 1:(f-1)*6 + 3)
       orig(2,1:3) = input%adj_ja(1,(f-1)*6 + 4:(f-1)*6 + 6)
       dest(1,1:3) = input%adj_ja(2,(f-1)*6 + 1:(f-1)*6 + 3)
       dest(2,1:3) = input%adj_ja(2,(f-1)*6 + 4:(f-1)*6 + 6)
       ...
       do concurrent( s = 1:size(output%val, dim=2) )
          ...
          case(3, 4) ! circular or reflection
             step1 = merge(-1, 1, input%indices(1) .eq. 4 .and. idim .eq. 1)
             step2 = merge(-1, 1, input%indices(1) .eq. 4 .and. idim .eq. 2)
             step3 = merge(-1, 1, input%indices(1) .eq. 4 .and. idim .eq. 3)
             do k = dest(1,3), dest(2,3)
                do j = dest(1,2), dest(2,2)
                   do i = dest(1,1), dest(2,1)
                      idx_out = i + (j-1) * output_h + &
          ...
          case(5) ! replication
             ...
             case(3) ! Edge along dimension 3
                do j = 1, input_w
                   do i = 1, input_h
                      idx_in = orig(1,1) + i - 1 + (j-1) * input_h
                      grad_sum = 0._real32
                      do k = dest(1,3), dest(2,3)
                         idx_out = i + (j-1) * output_h + &
                              (k-1) * output_h * output_w
...
!-------------------------------------------------------------------------------
  subroutine accumulate_face_gradients_3d(input, output)
...
!###############################################################################
  module function pad2d(input, facets, pad_size, imethod) result(output)
    ...
    output_shape = [ output_size_h, output_size_w, size(input%val, dim=2) ]
    output => input%create_result(array_shape = output_shape)
...
!-------------------------------------------------------------------------------
  function get_partial_pad2d(this, upstream_grad) result(output)
    ...
    if(this%indices(1) .ge. 3 .and. this%indices(1) .le. 5)then
       call accumulate_corner_gradients_2d( upstream_grad, output )
       call accumulate_edge_gradients_2d( upstream_grad, output )
    end if
...
!###############################################################################
  module function pad3d(input, facets, pad_size, imethod) result(output)
    ...
    integer :: i, j, k, s
    integer :: idx_in, idx_out, idx_shift
    integer :: input_size_h, input_size_w, input_size_d
    ...
    output_shape = [ output_size_h, output_size_w, output_size_d, &
    ...
    ! Edges (1D edges)
    do i = 1, facets(1)%num
       output%adj_ja(1,(i-1)*6 + 1 : (i-1)*6 + 3) = facets(1)%orig_bound(1,1:3,i)
       output%adj_ja(1,(i-1)*6 + 4 : (i-1)*6 + 6) = facets(1)%orig_bound(1,4:6,i)
       output%adj_ja(2,(i-1)*6 + 1 : (i-1)*6 + 3) = facets(1)%dest_bound(1,1:3,i)
       output%adj_ja(2,(i-1)*6 + 4 : (i-1)*6 + 6) = facets(1)%dest_bound(1,4:6,i)
    end do
    idx_shift = facets(1)%num * 6
    ! Faces (2D faces)
    do i = 1, facets(2)%num
       output%adj_ja(1,(i -1)*6 + 1 + idx_shift : (i -1)*6 + 3 + idx_shift) = &
            facets(2)%orig_bound(1,1:3,i)
       output%adj_ja(1,(i -1)*6 + 4 + idx_shift : (i -1)*6 + 6 + idx_shift) = &
            facets(2)%orig_bound(1,4:6,i)
       output%adj_ja(2,(i -1)*6 + 1 + idx_shift : (i -1)*6 + 3 + idx_shift) = &
            facets(2)%dest_bound(1,1:3,i)
       output%adj_ja(2,(i -1)*6 + 4 + idx_shift : (i -1)*6 + 6 + idx_shift) = &
            facets(2)%dest_bound(1,4:6,i)
    end do
    idx_shift = idx_shift + facets(2)%num * 6
    ! Corners (3D corners)
    do i = 1, facets(3)%num
       output%adj_ja(1,(i -1)*6 + 1 + idx_shift : (i -1)*6 + 3 + idx_shift) = &
            facets(3)%orig_bound(1,1:3,i)
       output%adj_ja(1,(i -1)*6 + 4 + idx_shift : (i -1)*6 + 6 + idx_shift) = &
            facets(3)%orig_bound(1,4:6,i)
       output%adj_ja(2,(i -1)*6 + 1 + idx_shift : (i -1)*6 + 3 + idx_shift) = &
            facets(3)%dest_bound(1,1:3,i)
       output%adj_ja(2,(i -1)*6 + 4 + idx_shift : (i -1)*6 + 6 + idx_shift) = &
            facets(3)%dest_bound(1,4:6,i)
    end do
...
!-------------------------------------------------------------------------------
  function get_partial_pad3d(this, upstream_grad) result(output)
    ...
    if(this%indices(1) .ge. 3 .and. this%indices(1) .le. 5)then
       call accumulate_corner_gradients_3d( upstream_grad, output )
       call accumulate_edge_gradients_3d( upstream_grad, output )
       call accumulate_face_gradients_3d( upstream_grad, output )
    end if
