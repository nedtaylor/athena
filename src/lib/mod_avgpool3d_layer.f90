module athena__avgpool3d_layer
  ...
  type, extends(pool_layer_type) :: avgpool3d_layer_type
   ...
   contains
     ...
     procedure, pass(this) :: forward_derived => forward_derived_avgpool3d
  ...
  subroutine set_batch_size_avgpool3d(this, batch_size, verbose)
    ...
    if(allocated(this%input_shape))then
       if(this%use_graph_input)then
          call stop_program( &
               "Graph input not supported for 3D average pooling layer" &
          )
          return
       end if
...
!###############################################################################
  subroutine forward_derived_avgpool3d(this, input)
    !! Forward propagation
    implicit none
  ...
  end subroutine forward_derived_avgpool3d
!###############################################################################
