...
!!!#############################################################################
  function read_batchnorm2d_layer(unit, verbose) result(layer)
    ...
    tag_loop: do
       ...
       if(stat.ne.0)then
          write(0,*) "ERROR: file encountered error (EoF?) before END BATCHNORM2D"
          stop "Exiting..."
       end if
    ...
    layer = batchnorm2d_layer_type( input_shape=input_shape, &
    ...
    if(trim(adjustl(buffer)).ne."END BATCHNORM2D")then
       ...
       stop "ERROR: END BATCHNORM2D not where expected"
    end if
