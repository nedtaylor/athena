...
module conv1d_layer
  ...
  type, extends(conv_layer_type) :: conv1d_layer_type
   ...
   contains
     ...
     procedure, pass(this) :: get_gradients => get_gradients_conv1d
     procedure, pass(this) :: set_gradients => set_gradients_conv1d
     procedure, pass(this) :: get_output => get_output_conv1d

     procedure, pass(this) :: init => init_conv1d
...
!!!#############################################################################
  function layer_add(a, b) result(output)
    implicit none
    class(conv1d_layer_type), intent(in) :: a, b
    type(conv1d_layer_type) :: output
...
!!!#############################################################################
  ...
  end function get_params_conv1d
!!!#############################################################################
...
!!!#############################################################################
  ...
  end subroutine set_params_conv1d
!!!#############################################################################
...
!!! procedure modified from neural-fortran library
!!!#############################################################################
  pure function get_gradients_conv1d(this, clip_method) result(gradients)
  ...
  end function get_gradients_conv1d
!!!#############################################################################
...
!!!#############################################################################
 ...
 end subroutine set_gradients_conv1d
!!!#############################################################################
...
!!! get layer outputs
!!!#############################################################################
  pure subroutine get_output_conv1d(this, output)
    implicit none
    class(conv1d_layer_type), intent(in) :: this
  ...
  end subroutine get_output_conv1d
!!!#############################################################################
...
!!!#############################################################################
  module function layer_setup( &
    ...
    if(present(calc_input_gradients))then
       ...
       write(*,*) "CONV1D input gradients turned off"
    else
    ...
    write(*,'("CONV1D activation function: ",A)') trim(activation_function_)
    ...
    write(*,'("CONV1D kernel initialiser: ",A)') trim(layer%kernel_initialiser)
    ...
    write(*,'("CONV1D bias initialiser: ",A)') trim(layer%bias_initialiser)
...
!!! initialise layer
!!!#############################################################################
  subroutine init_conv1d(this, input_shape, batch_size, verbose)
...
!!!#############################################################################
  function read_conv1d_layer(unit, verbose) result(layer)
    ...
    class(conv1d_layer_type), allocatable :: layer
...
!!!#############################################################################
  pure subroutine backward_3d(this, input, gradient)
    ...
    grad_dz = 0._real12
    grad_dz(&
         1:this%output_shape(1),:,:) = gradient * &
