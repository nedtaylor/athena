...
!###############################################################################
  subroutine set_hyperparams_maxpool3d( &
...
!###############################################################################
  subroutine build_from_onnx_maxpool3d( &
    ...
    do i = 1, size(node%attributes)
       ...
       case default
          ! Do nothing
          write(0,*) "WARNING: Unrecognised attribute in ONNX MAXPOOL3D ", &
    ...
    if(size(initialisers).ne.0)then
       write(0,*) "WARNING: initialisers not used for ONNX MAXPOOL3D layer"
    end if
  ...
  end subroutine build_from_onnx_maxpool3d
!###############################################################################
...
!###############################################################################
  subroutine forward_maxpool3d(this, input)
    ...
    case(.true.)
       ...
       ptr => maxpool3d(this%pad_layer%output(1,1), this%pool, this%strd)
    case default
