submodule (athena__diffstruc_extd) athena__diffstruc_extd_submodule_conv
  !! Submodule containing implementations for extended diffstruc array
  !! operations
...
!###############################################################################
  module function conv1d(input, kernel, stride, dilation) result(output)
    ...
    integer :: i_in, k_idx, k_base_idx, out_base_idx
    integer :: input_h, kernel_h, output_h, num_channels, num_filters
    integer :: batch_size, kernel_channel_size
    ...
    batch_size = size(input%val, dim=2)
    kernel_channel_size = kernel_h

    ! Calculate output dimensions
    output_h = (input_h - dilation*(kernel_h - 1) - 1) / &
         stride + 1
    output_shape = [output_h, num_filters, batch_size]
    ...
    do concurrent(s = 1:output_shape(3), c_out = 1:num_filters, &
       ...
       output%val(i + out_base_idx, s) = conv_sum
    end do
    ...
    if(requires_grad)then
       ...
       output%get_partial_right_val_sum => get_partial_conv1d_kernel_val_sum
       output%get_partial_both_val => get_partial_conv1d_mixed_val
...
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv1d_input_val(this, upstream_grad, output)
    ...
    ! Local variables
    integer :: i, k, c_in, c_out, s
    integer :: i_in, k_idx, out_idx, input_base_idx, kernel_base_idx
    integer :: input_h, kernel_h, output_h, num_channels, num_filters
...
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv1d_kernel_val(this, upstream_grad, output)
    ...
    output(:,1) = 0._real32
    call get_partial_conv1d_kernel_val_sum(this, upstream_grad, output(:,1))

  end subroutine get_partial_conv1d_kernel_val
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv1d_kernel_val_sum(this, upstream_grad, output)
    !! Sum-reduced partial derivative wrt kernel for 1D convolution
    implicit none
    ...
    integer :: i, k, c_in, c_out, s
    integer :: i_in, k_idx, out_idx, input_base_idx, out_base_idx
    integer :: input_h, kernel_h, output_h, num_channels, num_filters
    integer :: stride, dilation
    ...
    do concurrent(c_out = 1:num_filters, c_in = 1:num_channels, k = 1:kernel_h)
       ...
       do s = 1, size(upstream_grad, dim=2)
          do i = 1, output_h
             i_in = ( i - 1 ) * stride + ( k - 1 ) * dilation + 1
             if(i_in .ge. 1 .and. i_in .le. input_h)then
                out_idx = i + out_base_idx
                grad_sum = grad_sum + upstream_grad(out_idx, s) * &
                     this%left_operand%val(i_in + input_base_idx, s)
             end if
          end do
       end do
       output(k_idx) = grad_sum
    end do

  end subroutine get_partial_conv1d_kernel_val_sum
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv1d_mixed_val( &
       this, upstream_grad, left_output, right_output)
    !! Fused partial derivative wrt input and kernel for 1D convolution
    implicit none
    ...
    integer :: i, k, c_in, c_out, s
    integer :: i_in, out_idx, input_base_idx, kernel_base_idx, kernel_idx
    integer :: input_h, kernel_h, output_h, num_channels, num_filters
    integer :: stride, dilation
    ...
    left_output = 0._real32
    right_output = 0._real32
             ...
             if(abs(grad_val) .gt. 1.e-30_real32)then
                do c_in = 1, num_channels
                   ...
                   do k = 1, kernel_h
                      i_in = (i - 1) * stride + (k - 1) * dilation + 1
                      if(i_in .ge. 1 .and. i_in .le. input_h)then
                         kernel_idx = k + kernel_base_idx
                         left_output(i_in + input_base_idx, s) = &
                              left_output(i_in + input_base_idx, s) + &
                              grad_val * this%right_operand%val(kernel_idx, 1)
                         right_output(kernel_idx) = right_output(kernel_idx) + &
                              grad_val * this%left_operand%val( &
                                   i_in + input_base_idx, s)
                      end if
                   end do
                end do
             end if
  ...
  end subroutine get_partial_conv1d_mixed_val
!###############################################################################
...
!###############################################################################
  module function conv2d(input, kernel, stride, dilation) result(output)
    ...
    output_shape = [output_h, output_w, num_filters, batch_size]
    ...
    if(requires_grad)then
       ...
       output%get_partial_right_val_sum => get_partial_conv2d_kernel_val_sum
       output%get_partial_both_val => get_partial_conv2d_mixed_val
       output%requires_grad = .true.
       output%is_forward = input%is_forward
...
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv2d_kernel_val(this, upstream_grad, output)
    ...
    output(:,1) = 0._real32
    call get_partial_conv2d_kernel_val_sum(this, upstream_grad, output(:,1))

  end subroutine get_partial_conv2d_kernel_val
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv2d_kernel_val_sum(this, upstream_grad, output)
    !! Sum-reduced partial derivative wrt kernel for 2D convolution
    implicit none
  ...
  end subroutine get_partial_conv2d_kernel_val_sum
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv2d_mixed_val( &
       this, upstream_grad, left_output, right_output)
    !! Fused partial derivative wrt input and kernel for 2D convolution
    implicit none
    ...
    integer :: i, j, ki, kj, c_in, c_out, s
    integer :: i_in, j_in, out_idx, in_idx
    integer :: in_base_idx, out_base_idx, kernel_base_idx
    integer :: kernel_channel_size, input_h, input_w, kernel_h, kernel_w
    integer :: output_h, output_w, num_channels, num_filters
    integer, dimension(2) :: stride, dilation
    integer :: channel_size_in, channel_size_out, kernel_idx
    real(real32) :: grad_val
          ...
          do j = 1, output_w
             do i = 1, output_h
                ...
                if(abs(grad_val) .gt. 1.e-30_real32)then
                   do c_in = 1, num_channels
                      in_base_idx = (c_in - 1) * channel_size_in
                      kernel_base_idx = (c_in - 1) * kernel_channel_size + &
                           (c_out - 1) * kernel_channel_size * num_channels
                      do kj = 1, kernel_w
                         j_in = (j - 1) * stride(2) + &
                              (kj - 1) * dilation(2) + 1
                         if(j_in .ge. 1 .and. j_in .le. input_w)then
                            do ki = 1, kernel_h
                               ...
                               if(i_in .ge. 1 .and. i_in .le. input_h)then
                                  in_idx = i_in + (j_in - 1) * input_h + &
                                       in_base_idx
                                  kernel_idx = ki + (kj - 1) * kernel_h + &
                                       kernel_base_idx
                                  right_output(kernel_idx) = &
                                       right_output(kernel_idx) + grad_val * &
                                       this%left_operand%val(in_idx, s)
                                  kernel_idx = (kernel_h - ki + 1) + &
                                       (kernel_w - kj) * kernel_h + &
                                       kernel_base_idx
                                  left_output(in_idx, s) = &
                                       left_output(in_idx, s) + grad_val * &
                                       this%right_operand%val( &
                                            kernel_idx, 1)
                               end if
                            end do
                         end if
  ...
  end subroutine get_partial_conv2d_mixed_val
!###############################################################################
...
!###############################################################################
  module function conv2d_relu(input, kernel, bias, stride, dilation) result(output)
    ...
    do concurrent(s = 1:output_shape(4), c_out = 1:num_filters, &
       ...
       out_idx = i + (j - 1) * output_h + (c_out - 1) * channel_size_out
       conv_sum = conv_sum + bias%val(c_out, 1)
       if(conv_sum .gt. 0._real32)then
          output%val(out_idx, s) = conv_sum
       else
          output%val(out_idx, s) = 0._real32
       end if
    end do
    ...
    requires_grad = input%requires_grad .or. kernel%requires_grad .or. &
         bias%requires_grad
    if(requires_grad)then
       packed_params => pack_conv2d_relu_params(kernel, bias)
       allocate(output%indices(2))
       output%indices(1) = num_channels
       output%indices(2) = num_filters
       allocate(output%adj_ja(2,3))
       ...
       output%adj_ja(1,3) = kernel_h
       output%adj_ja(2,3) = kernel_w
       output%get_partial_left_val => get_partial_conv2d_relu_input_val
       output%get_partial_right_val => get_partial_conv2d_relu_params_val
       output%get_partial_right_val_sum => get_partial_conv2d_relu_params_val_sum
       output%get_partial_both_val => get_partial_conv2d_relu_both_val
       output%requires_grad = .true.
       output%is_forward = input%is_forward .or. kernel%is_forward .or. &
            bias%is_forward
       output%operation = 'conv2d_relu'
       output%left_operand => input
...
!-------------------------------------------------------------------------------
  function pack_conv2d_relu_params(kernel, bias) result(output)
    implicit none
    ...
    output%get_partial_left => get_partial_pack_conv2d_relu_left
    output%get_partial_right => get_partial_pack_conv2d_relu_right
    output%get_partial_left_val => get_partial_pack_conv2d_relu_left_val
    output%get_partial_right_val => get_partial_pack_conv2d_relu_right_val
    if(kernel%requires_grad .or. bias%requires_grad)then
       output%requires_grad = .true.
       output%is_forward = kernel%is_forward .or. bias%is_forward
       output%operation = 'pack_conv2d_relu_params'
  ...
  end function pack_conv2d_relu_params
!-------------------------------------------------------------------------------
  function get_partial_pack_conv2d_relu_left(this, upstream_grad) result(output)
    implicit none
    ...
    call output%allocate(array_shape = [this%left_operand%shape, 1])
    call this%get_partial_left_val(upstream_grad%val, output%val)

  end function get_partial_pack_conv2d_relu_left
!-------------------------------------------------------------------------------
  function get_partial_pack_conv2d_relu_right(this, upstream_grad) result(output)
    implicit none
  ...
  end function get_partial_pack_conv2d_relu_right
!-------------------------------------------------------------------------------
  pure subroutine get_partial_pack_conv2d_relu_left_val(this, upstream_grad, output)
    implicit none
    ...
    output(:,1) = upstream_grad(1:this%indices(1),1)

  end subroutine get_partial_pack_conv2d_relu_left_val
!-------------------------------------------------------------------------------
  pure subroutine get_partial_pack_conv2d_relu_right_val(this, upstream_grad, output)
    implicit none
    ...
    output(:,1) = upstream_grad(this%indices(1)+1:,1)

  end subroutine get_partial_pack_conv2d_relu_right_val
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv2d_relu_input_val(this, upstream_grad, output)
    implicit none
    ...
    integer :: i, j, ki, kj, c_in, c_out, s
    integer :: i_in, j_in, in_idx, out_idx, in_base_idx, kernel_base_idx
    integer :: kernel_idx, input_h, input_w, kernel_h, kernel_w
    integer :: output_h, output_w, num_channels, num_filters
    integer :: channel_size_in, channel_size_out, kernel_channel_size
    integer :: stride(2), dilation(2)
    real(real32) :: grad_val
    ...
    do s = 1, size(upstream_grad, dim=2)
       do c_out = 1, num_filters
          do j = 1, output_w
             do i = 1, output_h
                out_idx = i + (j - 1) * output_h + (c_out - 1) * channel_size_out
                if(this%val(out_idx, s) .gt. 0._real32)then
                   grad_val = upstream_grad(out_idx, s)
                   if(abs(grad_val) .gt. 1.e-30_real32)then
                      do c_in = 1, num_channels
                         in_base_idx = (c_in - 1) * channel_size_in
                            ...
                            if(j_in .ge. 1 .and. j_in .le. input_w)then
                               do ki = 1, kernel_h
                                  ...
                                  if(i_in .ge. 1 .and. i_in .le. input_h)then
                                     in_idx = i_in + (j_in - 1) * input_h + in_base_idx
                                     kernel_idx = (kernel_h - ki + 1) + &
                                          (kernel_w - kj) * kernel_h + kernel_base_idx
                                     output(in_idx, s) = output(in_idx, s) + &
                                          grad_val * this%right_operand%val(kernel_idx, 1)
                                  end if
                               end do
  ...
  end subroutine get_partial_conv2d_relu_input_val
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv2d_relu_params_val(this, upstream_grad, output)
    implicit none
    ...
    output(:,1) = 0._real32
    call get_partial_conv2d_relu_params_val_sum(this, upstream_grad, output(:,1))

  end subroutine get_partial_conv2d_relu_params_val
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv2d_relu_params_val_sum(this, upstream_grad, output)
    implicit none
    ...
    integer :: output_h, output_w, num_channels, num_filters
    integer :: channel_size_in, channel_size_out, kernel_channel_size
    integer :: stride(2), dilation(2), split_idx
    real(real32) :: grad_val
       ...
       do c_out = 1, num_filters
          do j = 1, output_w
             do i = 1, output_h
                ...
                if(this%val(out_idx, s) .gt. 0._real32)then
                   grad_val = upstream_grad(out_idx, s)
                   if(abs(grad_val) .gt. 1.e-30)then
                      output(split_idx + c_out) = output(split_idx + c_out) + grad_val
                      do c_in = 1, num_channels
                         in_base_idx = (c_in - 1) * channel_size_in
                         kernel_base_idx = (c_in - 1) * kernel_channel_size + &
                              (c_out - 1) * kernel_channel_size * num_channels
                         do kj = 1, kernel_w
                            ...
                            if(j_in .ge. 1 .and. j_in .le. input_w)then
                               do ki = 1, kernel_h
                                  ...
                                  if(i_in .ge. 1 .and. i_in .le. input_h)then
                                     in_idx = i_in + (j_in - 1) * input_h + in_base_idx
                                     kernel_idx = ki + (kj - 1) * kernel_h + kernel_base_idx
                                     output(kernel_idx) = output(kernel_idx) + &
                                          grad_val * this%left_operand%val(in_idx, s)
                                  end if
                               end do
                            end if
                         end do
  ...
  end subroutine get_partial_conv2d_relu_params_val_sum
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv2d_relu_both_val(this, upstream_grad, left_output, right_output)
    implicit none
          ...
          do j = 1, output_w
             do i = 1, output_h
                ...
                if(this%val(out_idx, s) .gt. 0._real32)then
                   grad_val = upstream_grad(out_idx, s)
                   if(abs(grad_val) .gt. 1.e-30_real32)then
                      right_output(split_idx + c_out) = right_output(split_idx + c_out) + grad_val
                      do c_in = 1, num_channels
                         in_base_idx = (c_in - 1) * channel_size_in
                         kernel_base_idx = (c_in - 1) * kernel_channel_size + &
                              (c_out - 1) * kernel_channel_size * num_channels
                            ...
                            if(j_in .ge. 1 .and. j_in .le. input_w)then
                               do ki = 1, kernel_h
                                  ...
                                  if(i_in .ge. 1 .and. i_in .le. input_h)then
                                     ...
                                     left_output(in_idx, s) = &
                                          left_output(in_idx, s) + grad_val * &
                                          this%right_operand%val(kernel_idx, 1)
                                  end if
                               end do
                            end if
                         end do
  ...
  end subroutine get_partial_conv2d_relu_both_val
...
!###############################################################################
  module function conv3d(input, kernel, stride, dilation) result(output)
    ...
    integer :: k_base_idx, out_base_idx, input_plane_size
    ...
    integer :: channel_size_in, channel_size_out
    integer :: kernel_plane_size, kernel_channel_size
    integer :: dil_kernel_h_m1, dil_kernel_w_m1, dil_kernel_d_m1
    integer :: batch_size
    real(real32) :: conv_sum
    ...
    input_plane_size = input_h * input_w
    channel_size_in = input_plane_size * input_d
    kernel_plane_size = kernel_h * kernel_w
    kernel_channel_size = kernel_plane_size * kernel_d
    dil_kernel_h_m1 = dilation(1) * (kernel_h - 1)
    dil_kernel_w_m1 = dilation(2) * (kernel_w - 1)
    dil_kernel_d_m1 = dilation(3) * (kernel_d - 1)

    ! Calculate output dimensions
    output_h = (input_h - dil_kernel_h_m1 - 1) / &
         stride(1) + 1
    output_w = (input_w - dil_kernel_w_m1 - 1) / &
         stride(2) + 1
    output_d = (input_d - dil_kernel_d_m1 - 1) / &
         stride(3) + 1
                ...
                do kj = 1, kernel_w
                   ...
                   if(j_in .ge. 1 .and. j_in .le. input_w)then
                      do ki = 1, kernel_h
                         ...
                         if(i_in .ge. 1 .and. i_in .le. input_h)then
                            in_idx = i_in + ( j_in - 1 ) * input_h + &
                                 ( k_in - 1 ) * input_plane_size + &
                                 ( c_in - 1 ) * channel_size_in
                            k_idx = ki + ( kj - 1 ) * kernel_h + &
                                 ( kk - 1 ) * kernel_plane_size + &
                                 k_base_idx
                            conv_sum = conv_sum + input%val(in_idx, s) * &
                                 kernel%val(k_idx, 1)
                         end if
    ...
    if(requires_grad)then
       ...
       output%get_partial_right_val_sum => get_partial_conv3d_kernel_val_sum
       output%get_partial_both_val => get_partial_conv3d_mixed_val
       output%requires_grad = .true.
       output%is_forward = input%is_forward
...
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv3d_input_val(this, upstream_grad, output)
    ...
    integer :: in_base_idx, out_base_idx, k_base_idx
    integer :: input_plane_size, output_plane_size
    integer :: kernel_plane_size, kernel_channel_size
    real(real32) :: grad_val, kernel_val
    ...
    input_plane_size = input_h * input_w
    output_plane_size = output_h * output_w
    channel_size_in = input_plane_size * input_d
    channel_size_out = output_plane_size * output_d
    kernel_plane_size = kernel_h * kernel_w
    kernel_channel_size = kernel_plane_size * kernel_d
                                     ...
                                     do ki = 1, kernel_h
                                        ...
                                        if( i_in .ge. 1 .and. &
                                           ...
                                           k_idx = ki + &
                                                ...
                                                k_base_idx
                                           kernel_val = &
                                                this%right_operand%val(k_idx, 1)
                                           output(in_idx, s) = &
                                                output(in_idx, s) + &
                                                grad_val * kernel_val
                                        end if
...
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv3d_kernel_val(this, upstream_grad, output)
    ...
    output(:,1) = 0._real32
    call get_partial_conv3d_kernel_val_sum(this, upstream_grad, output(:,1))

  end subroutine get_partial_conv3d_kernel_val
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv3d_kernel_val_sum(this, upstream_grad, output)
    !! Sum-reduced partial derivative wrt kernel for 3D convolution
    implicit none
    ...
    do concurrent(c_out = 1:num_filters, c_in = 1:num_channels, &
         kk = 1:kernel_d, kj = 1:kernel_w, ki = 1:kernel_h)
       in_base_idx = (c_in - 1) * channel_size_in
       out_base_idx = (c_out - 1) * channel_size_out
       k_base_idx = (c_in - 1) * kernel_channel_size + &
            (c_out - 1) * kernel_channel_size * num_channels
       k_idx = ki + (kj - 1) * kernel_h + &
            (kk - 1) * kernel_plane_size + k_base_idx
                ...
                do j = 1, output_w
                   ...
                   if(j_in .ge. 1 .and. j_in .le. input_w)then
                      do i = 1, output_h
                         ...
                         if(i_in .ge. 1 .and. i_in .le. input_h)then
                            in_idx = i_in + (j_in - 1) * input_h + &
                                 (k_in - 1) * input_plane_size + in_base_idx
                            out_idx = i + (j - 1) * output_h + &
                                 (k - 1) * output_plane_size + out_base_idx
                            grad_sum = grad_sum + &
  ...
  end subroutine get_partial_conv3d_kernel_val_sum
!-------------------------------------------------------------------------------
  pure subroutine get_partial_conv3d_mixed_val( &
       this, upstream_grad, left_output, right_output)
    !! Fused partial derivative wrt input and kernel for 3D convolution
    implicit none
    ...
    integer :: i, j, k, ki, kj, kk, c_in, c_out, s
    integer :: i_in, j_in, k_in, out_idx, in_idx, kernel_idx
    integer :: input_h, input_w, input_d, kernel_h, kernel_w, kernel_d
    integer :: output_h, output_w, output_d, num_channels, num_filters
    integer :: channel_size_in, channel_size_out
    integer :: input_plane_size, output_plane_size
    integer :: kernel_plane_size, kernel_channel_size
    integer, dimension(3) :: stride, dilation
  ...
  end subroutine get_partial_conv3d_mixed_val
!###############################################################################
