  ...
  subroutine set_hyperparams_batchnorm3d( &
    ...
    if(present(verbose))then
       if(abs(verbose).gt.0)then
          write(*,'("BATCHNORM3D gamma (kernel) initialiser: ",A)') &
               trim(this%kernel_init%name)
          write(*,'("BATCHNORM3D beta (bias) initialiser: ",A)') &
...
!###############################################################################
  subroutine build_from_onnx_batchnorm3d( &
    ...
    do i = 1, size(node%attributes)
       ...
       case default
          ! Do nothing
          write(0,*) "WARNING: Unrecognised attribute in ONNX BATCHNORM3D &
    ...
    if(size(initialisers).ne.4)then
       call stop_program("ONNX BATCHNORM3D layer requires 4 initialisers &
  ...
  end subroutine build_from_onnx_batchnorm3d
!###############################################################################
