module athena__batchnorm0d_layer
  !! Module containing implementation of 0D and 0D batch normalisation layers
  !!
  ...
  public :: batchnorm0d_layer_type
  public :: read_batchnorm0d_layer


  type, extends(batch_layer_type) :: batchnorm0d_layer_type
     !! Type for 0D or 0D batch normalisation layer with overloaded procedures
   contains
     procedure, pass(this) :: set_hyperparams => set_hyperparams_batchnorm0d
     !! Set hyperparameters for 0D batch normalisation layer
     procedure, pass(this) :: read => read_batchnorm0d
     !! Read 0D batch normalisation layer from file

     procedure, pass(this) :: forward => forward_batchnorm0d
     !! Forward propagation derived type handler

     final :: finalise_batchnorm0d
     !! Finalise 0D batch normalisation layer
  end type batchnorm0d_layer_type


  interface batchnorm0d_layer_type
     !! Interface for setting up the 0D batch normalisation layer
     module function layer_setup( &
       ...
       !! Set up the 0D batch normalisation layer
       ...
       !! Verbosity level
       type(batchnorm0d_layer_type) :: layer
       !! Instance of the 0D batch normalisation layer
     end function layer_setup
  end interface batchnorm0d_layer_type
...
!###############################################################################
  subroutine finalise_batchnorm0d(this)
    !! Finalise 0D batch normalisation layer
    implicit none

    ! Arguments
    type(batchnorm0d_layer_type), intent(inout) :: this
    !! Instance of the 0D batch normalisation layer
  ...
  end subroutine finalise_batchnorm0d
!###############################################################################
...
!###############################################################################
  subroutine set_hyperparams_batchnorm0d( &
       this, &
       momentum, epsilon, &
       gamma_init_mean, gamma_init_std, &
       beta_init_mean, beta_init_std, &
    ...
    ! Arguments
    class(batchnorm0d_layer_type), intent(inout) :: this
    !! Instance of the 0D batch normalisation layer
    ...
    if(present(verbose))then
       if(abs(verbose).gt.0)then
          write(*,'("BATCHNORM0D gamma (kernel) initialiser: ",A)') &
               trim(this%kernel_init%name)
          write(*,'("BATCHNORM0D beta (bias) initialiser: ",A)') &
               trim(this%bias_init%name)
          write(*,'("BATCHNORM0D moving mean initialiser: ",A)') &
               trim(this%moving_mean_init%name)
          write(*,'("BATCHNORM0D moving variance initialiser: ",A)') &
               trim(this%moving_variance_init%name)
       end if
    end if

  end subroutine set_hyperparams_batchnorm0d
!###############################################################################
...
!###############################################################################
  subroutine read_batchnorm0d(this, unit, verbose)
    !! Read 0D batch normalisation layer from file
  ...
  end subroutine read_batchnorm0d
!###############################################################################


!###############################################################################
  function read_batchnorm0d_layer(unit, verbose) result(layer)
    !! Read 0D batch normalisation layer from file and return layer
    implicit none
    ...
    if(present(verbose)) verbose_ = verbose
    allocate(layer, source=batchnorm0d_layer_type())
    call layer%read(unit, verbose=verbose_)

  end function read_batchnorm0d_layer
!###############################################################################


!###############################################################################
  subroutine build_from_onnx_batchnorm0d( &
       this, node, initialisers, value_info, verbose &
  )
    !! Read ONNX attributes for 0D batch normalisation layer
    ...
    do i = 1, size(node%attributes)
       ...
       case default
          ! Do nothing
          write(0,*) "WARNING: Unrecognised attribute in ONNX BATCHNORM0D &
    ...
    if(size(initialisers).ne.4)then
       call stop_program("ONNX BATCHNORM0D layer requires 4 initialisers &
  ...
  end subroutine build_from_onnx_batchnorm0d
!###############################################################################
...
!###############################################################################
  subroutine forward_batchnorm0d(this, input)
    !! Forward propagation
    implicit none

    ! Arguments
    class(batchnorm0d_layer_type), intent(inout), target :: this
    !! Instance of the 0D batch normalisation layer
  ...
  end subroutine forward_batchnorm0d
!###############################################################################

end module athena__batchnorm0d_layer
