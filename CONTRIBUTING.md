...
Each library source file contains one module and possibly multiple submodules.
The source files that define the submodule end with `_sub[_.*].f90`.

## Components

For a more in-depth description of athena, guides for its core components, and fully compiled API documentation, refer to the [Read*the*Docs](https://athena-fortran.readthedocs.io/).
...
The type also contains optimisation variables including `optimiser` and `metrics`.
Finally, within the `network_type` is the `model` variable, which is an allocatable array of type `type(container_layer_type)`.

### Containers

Layer containers have a single use; they allow for the allocation of an exact layer type to the enclosed `layer` variable.
This enables one to have an array of containers, each with a different extended version of the ``base_layer_type``.
...
To ensure full compatibility of the custom layer with the athena framework, please refer to [mod_container_layer](src/lib/mod_container_layer.f90) and its submodule, specifically ``list_of_layer_types`` and ``list_of_onnx_layer_creators`` that should be extended to ensure athena can properly save and load the custom layer.
...
An optimiser is either the base type `base_optimiser_type` or an extended type.
The type is stored in the network variable `network_type%optimiser` and can either be set manually or passed to the network during the network compilation call, `network_type%compile()`.
