...
The ``batchnorm2d_layer_type`` derived type provides a 2D batch normalisation layer.
This layer applies batch normalisation over 4D inputs (width, height, channels, batch), normalizing over the spatial dimensions and batch.
...
* **input_shape** (`integer, dimension(:)`): Shape of the input data (channels, height, width).

Shape:
------

* Input: ``(width, height, num_channels, batch_size)``.
* Output: ``(width, height, num_channels, batch_size)``.
