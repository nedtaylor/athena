module custom_laplace_layer
  !> Athena-ready custom Laplace layer scaffolding.
  !!
  !! This module collects the Python-matched Laplace layer metadata into one
  !! place so it can be lifted into Athena without carrying the surrounding
  !! Cattaneo predictor blocks. The types here describe the learnable
  !! parameterisation and its initialisation helpers in a layer-oriented form
  !! consistent with Athena naming conventions.
  ...
  public :: custom_laplace_layer_type
  public :: custom_laplace_block_type

  real(real32), parameter :: pi_real32 = 3.14159265358979323846_real32

  !> Python-style Laplace layer parameter container.
  !!
  !! The parameterisation mirrors the Python layer used by the custom model:
  !! bounded polar weights, learnable positive poles, an input-dependent pole
  !! offset MLP, cached spectral-filter coefficients, and a normalized distance
  !! matrix for kernel construction.
  type :: custom_laplace_layer_type
     !> Number of input channels.
     integer :: in_channels = 0
     !> Number of output channels.
     integer :: out_channels = 0
     !> Number of Laplace poles / modes.
     integer :: modes = 0
     !> Grid size for the cached distance matrix.
     integer :: grid_size = 0
     !> Upper bound on polar amplitudes.
     real(real32) :: max_amp = 1.0_real32
     !> Sigmoid sharpness used to bound amplitudes.
     real(real32) :: amp_sharpness = 1.0_real32
     !> Scale factor for the pole-offset MLP output.
     real(real32) :: pole_offset_scale = 0.1_real32
     !> Lower clamp on positive pole values.
     real(real32) :: pole_min = 0.1_real32
     !> Upper clamp on positive pole values.
     real(real32) :: pole_max = 100.0_real32
     !> Strength parameter for the spectral filter.
     real(real32) :: filter_strength = 4.0_real32
     !> Safety factor for causal masking.
     real(real32) :: causal_safety = 1.0_real32
     !> Whether the causal mask is enabled.
     logical :: use_causal_mask = .true.
     !> Name of the spectral filter family.
     character(len=32) :: spectral_filter = 'exponential'
     !> Log-amplitude parameters in polar form.
     real(real32), allocatable :: weight_log_amp(:,:,:)
     !> Phase parameters in polar form.
     real(real32), allocatable :: weight_phase(:,:,:)
     !> Base learnable log-poles.
     real(real32), allocatable :: log_poles(:)
     !> Pole-offset MLP weights.
     real(real32), allocatable :: pole_mlp_w(:,:)
     !> Pole-offset MLP biases.
     real(real32), allocatable :: pole_mlp_b(:)
     !> Cached spectral-filter values.
     real(real32), allocatable :: filter_values(:)
     !> Cached pairwise distance matrix on the normalized grid.
     real(real32), allocatable :: dist(:,:)
   contains
     procedure :: init => init_custom_laplace_layer
       procedure :: forward => forward_custom_laplace_layer
     procedure :: num_params => num_params_custom_laplace_layer
  end type custom_laplace_layer_type

  !> Athena block wrapper for the custom Laplace layer.
  !!
  !! The block keeps the Athena pointwise 1x1 projection together with the
  !! Python-matched Laplace metadata so that the entire spectral block can be
  !! transferred into Athena as one unit.
  type :: custom_laplace_block_type
     !> Pointwise pre-spectral projection.
     type(conv1d_layer_type) :: pointwise
     !> Custom Laplace spectral parameterisation.
     type(custom_laplace_layer_type) :: spectral
     !> Whether runtime code still needs explicit InstanceNorm1d.
     logical :: requires_custom_instance_norm = .true.
     !> Whether the spectral mapping is still approximate.
     logical :: spectral_mapping_is_approximate = .false.
   contains
     procedure :: init => init_custom_laplace_block
       procedure :: forward => forward_custom_laplace_block
     procedure :: num_params => num_params_custom_laplace_block
  end type custom_laplace_block_type

contains

  !> Initialise the custom Laplace layer metadata.
  !!
  !! The initialisation follows the Python defaults: log-spaced poles, bounded
  !! polar amplitudes, normally distributed phases, a small pole-offset MLP,
  !! and cached filter and distance tensors for later runtime use.
  subroutine init_custom_laplace_layer(this, in_channels, out_channels, modes, grid_size, &
       max_amp, amp_sharpness, pole_offset_scale, pole_min, pole_max, &
       spectral_filter, filter_strength, use_causal_mask, causal_safety)
    class(custom_laplace_layer_type), intent(inout) :: this
    ...
    integer :: k
    real(real32) :: target_amp, target_sigmoid, init_bias, init_scale
    real(real32) :: mlp_scale
  ...
  end subroutine init_custom_laplace_layer

  !> Return the number of learnable Laplace-layer parameters.
  integer function num_params_custom_laplace_layer(this)
    class(custom_laplace_layer_type), intent(in) :: this

    num_params_custom_laplace_layer = 0
    if (allocated(this%weight_log_amp)) num_params_custom_laplace_layer = num_params_custom_laplace_layer + size(this%weight_log_amp)
    if (allocated(this%weight_phase)) num_params_custom_laplace_layer = num_params_custom_laplace_layer + size(this%weight_phase)
    if (allocated(this%log_poles)) num_params_custom_laplace_layer = num_params_custom_laplace_layer + size(this%log_poles)
    if (allocated(this%pole_mlp_w)) num_params_custom_laplace_layer = num_params_custom_laplace_layer + size(this%pole_mlp_w)
    if (allocated(this%pole_mlp_b)) num_params_custom_laplace_layer = num_params_custom_laplace_layer + size(this%pole_mlp_b)
  end function num_params_custom_laplace_layer

  subroutine forward_custom_laplace_layer(this, x, dt_star, dx_star, c_star, output)
    class(custom_laplace_layer_type), intent(in) :: this
  ...
  end subroutine forward_custom_laplace_layer

  !> Initialise an Athena LNO block with the custom Laplace metadata.
  subroutine init_custom_laplace_block(this, channels, modes, grid_size, activation, max_amp, amp_sharpness, &
       pole_offset_scale, pole_min, pole_max, spectral_filter, filter_strength, &
       use_causal_mask, causal_safety)
    class(custom_laplace_block_type), intent(inout) :: this
  ...
  end subroutine init_custom_laplace_block

  !> Return the parameter count for one LNO block.
  integer function num_params_custom_laplace_block(this)
    class(custom_laplace_block_type), intent(in) :: this

    num_params_custom_laplace_block = this%pointwise%get_num_params() + this%spectral%num_params()
  end function num_params_custom_laplace_block

   subroutine forward_custom_laplace_block(this, x, dt_star, dx_star, c_star, output)
      class(custom_laplace_block_type), intent(inout) :: this
      real(real32), intent(in) :: x(:,:,:)
      real(real32), intent(in), optional :: dt_star, dx_star
      real(real32), intent(in), optional :: c_star(:)
   ...
   end subroutine forward_custom_laplace_block

  !> Build the cached spectral filter coefficients.
  ...
  !> Build the normalized pairwise distance matrix on the grid.
  ...
  !> Fill a rank-2 array with normal random values.
  ...
  !> Fill a rank-3 array with normal random values.
...
end module custom_laplace_layer