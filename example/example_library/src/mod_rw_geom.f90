...
!!!#############################################################################
  subroutine geom_read(UNIT,lat,bas,length)
    ...
    case(4)
       write(0,'("ERROR: librarby not yet set up for CRYSTAL")')
       stop
    case(5)
...
!!! AND GIVE IT THE CASTEP AND QE OPTION OF LABC !!!
    ...
    case(4)
       write(0,'("ERROR: library not yet set up for CRYSTAL")')
       stop
    case(5)
...
!!!#############################################################################
  subroutine VASP_geom_write(UNIT,lat_write,bas_write,lcart)
    ...
    write(UNIT,trim(adjustl(fmt))) &
         (adjustl(bas_write%spec(j)%name),j=1,bas_write%nspec)
